...
get_stats() ->
  riak_core_stat:get_stats(?APP). % TODO: redirect
...
get_stat(Name, SummaryName) ->
  Path = [?PFX, ?APP] ++ Name,
  case exometer:get_value(Path, SummaryName) of % tODO: redirect
...
initialize_fuse_stats(Name) ->
  fuse_stats_exometer:init(Name). % TODO: Fuze stats?
...
%% @doc Create dynamic stats for search index. % TODO: What is this
...
perform_update({index_end, BatchSize, Time}) -> % TODO: create update(arg) -> r_s_mngr
...
create({fuse_recovered, Index}) ->
  FuseName = yz_fuse:fuse_name_for_index(Index), % TODO: investigate fuse and if that needs some abstraction
...
delete({fuse_recovered, Index}) ->
  exometer:delete([fuse, Index, recovered]); % TODO: redirect
delete(_Stat) ->
  ok.
...
update(StatUpdate) ->
  ...
  ok. % TODO: how does YZ stat worker work
